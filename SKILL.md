...
19. 后检查只关注报告中绘图相关内容的渲染质量: 检查内联绘制的图示, 图表, 插入的图片等是否清晰, 完整, 尺寸合适, 没有被截断或严重变形. 表格只检查是否使用了 `columns: n` (数字) 而非 `columns: (a, b, c, ...)` (显式比例元组). 不要主动调整字体大小, 页面 break, 段落间距, 版式比例等与绘图无关的排版参数. 当修改不涉及绘图, 表格或新增内容时, 可跳过 PDF 检查.
20. 完成前再次检查语气, 标点, 数学语法, 列表样式, 标题结构, 元数据字段, 资源路径和可复现实物.
...
- 以渲染后的 PDF 页面作为图表, 图示, 插入图片等绘图元素尺寸与完整性的最终裁决依据.
...
- 最终质检时, 通过页面图像检查编译后的 PDF, 仅当图表, 图示或插入图片等绘图元素在视觉上存在截断, 模糊, 严重变形或尺寸严重失调时才继续调整. 不要因字体大小, 分页位置或段落版式等非绘图问题反复修改布局.
